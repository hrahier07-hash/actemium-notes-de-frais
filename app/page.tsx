    ...
    <div className="acm-shell">
      ...
      <div className="acm-main-wrapper">
        <div className="acm-topbar">
          {/* Logo mobile only */}
          <div className="acm-topbar-logo-mobile" style={{ display: 'none', alignItems: 'center', gap: 8 }}>
            <img src={LOGO_SRC} alt="Actemium" style={{ height: 28, width: 'auto', mixBlendMode: 'multiply' }} />
            <span style={{ fontSize: 10, fontWeight: 700, color: 'var(--primary)', textTransform: 'uppercase', letterSpacing: '.06em' }}>Notes de frais</span>
          </div>
          {/* Page title */}
          ...
          <>
            ...
            {tab === 'export' && (() => {
              ...
              async function doExcelExport() {
                // xlsx-js-style : fork de xlsx avec support des styles côté browser
                const XLSX = await import('xlsx-js-style')

                // Couleurs texte par imputation (ARGB sans #)
                const TEXT_COLORS: Record<string, string> = {
                  '#a8e6a3': 'FF000000', // Vélizy → noir
                  '#a3d4f5': 'FF1A56DB', // Chanteloup → bleu
                  '#fde89a': 'FFC05C00', // Verneuil → orange
                  '#f5b8c8': 'FFC2185B', // Cantine → rose
                }
                ...
                const HEADER_STYLE = {
                  ...
                  border: {
                    bottom: { style: 'thin', color: { rgb: '1A5FB8' } }
                  }
                }

                const headers = ['Nom', 'Prénom', 'Repas payés', 'Repas invité', 'Total', 'Commentaires']

                const headerRow = headers.map(h => ({ v: h, t: 's', s: HEADER_STYLE }))

                const dataRows = rows.map(r => {
                  const rgb = (TEXT_COLORS[r.countColor] || 'FF000000').replace('FF','')
                  const cellStyle = (bold = false) => ({
                    font: { color: { rgb }, bold, sz: 10 },
                    alignment: { vertical: 'center', wrapText: false }
                  })
                  return [
                    { v: r.emp.nom,    t: 's', s: cellStyle() },
                    { v: r.emp.prenom, t: 's', s: cellStyle() },
                    { v: r.paye,       t: 'n', s: cellStyle() },
                    { v: r.invite,     t: 'n', s: cellStyle() },
                    { v: r.total,      t: 'n', s: cellStyle(true) },
                    { v: r.comment,    t: 's', s: cellStyle() },
                  ]
                })
                ...
                <div style={S.card} className="acm-card-mobile">
                  ...
                  {filteredEmpForAdmin.map((e, i) => (
                    <div key={e.id} className="acm-emp-row" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '13px 0', borderTop: i > 0 ? '1px solid var(--border)' : 'none' }}>
                      <div>
                        <div style={{ fontWeight: 500, color: 'var(--text)' }}>{e.nom} {e.prenom}</div>
      ...
      <style>{`
        ...
        @media (max-width: 640px) {
          .acm-topbar-logo-mobile { display: flex !important; }
...
function MealRow({ meal, empName, onEdit, onDelete }: { meal: Meal; empName: string; onEdit: () => void; onDelete: () => void }) {
  return (
    <div style={S.tableRow} className="acm-tr">
      <span style={{ fontSize: 12, color: 'var(--text3)', fontFamily: 'monospace' }}>{formatDateShort(meal.date)}</span>
...
const S = {
  ...
  tableHead: { display: 'grid', gridTemplateColumns: '110px 1fr 100px 64px', gap: 12, padding: '10px 14px 12px', borderBottom: '1px solid var(--border)', fontSize: 11, fontWeight: 700, color: 'var(--text3)', textTransform: 'uppercase', letterSpacing: '.08em', minWidth: 380 } as React.CSSProperties,
  tableRow: { display: 'grid', gridTemplateColumns: '110px 1fr 100px 64px', gap: 12, alignItems: 'center', padding: '13px 14px', borderBottom: '1px solid var(--border)', transition: 'background .1s', minWidth: 380 } as React.CSSProperties,
